...
def detail(request, section_no, id_no, random_string):
	...
	#Check if logged in user is Admin
	if(request.user.username=='admin' or request.user.username=='hydra'):
		return HttpResponseRedirect('/adminmain')
		...
		#GET request
			...
			id1=(int)(id_no)
			# if(section_no=='1'):
			# 	ques=SolvedQ1.objects.filter(Q(id_no=request.user)).order_by('q_id')
			# 	ques1=SolvedQ2.objects.filter(Q(id_no=request.user)).order_by('q_id')
			# 	ques2=SolvedQ3.objects.filter(Q(id_no=request.user)).order_by('q_id')
			# 	print("Ending")
				
			# 	print(id1)
			# 	print(len(ques),len(ques1),len(ques2),len(Section1.objects.all()) )
			# 	if(len(ques)<len(Section1.objects.all()) and id1 ==1):
			# 		return HttpResponseRedirect('/detail/Section/1/1/1')
			# 	elif(len(ques)<len(Section1.objects.all()) and id1 == (len(ques)+1)):
			# 		return HttpResponseRedirect('/detail/Section/1/'+str(len(ques)+1)+"/1")
			# 	elif(len(ques1)<len(Section2.objects.all()) and id1 != (len(ques)+1)):
			# 		return HttpResponseRedirect('/detail/Section/2/'+str(len(ques)+1)+"/1")
			# 	elif(len(ques2)<len(Section3.objects.all()) and id1 != (len(ques)+1)):
			# 		return HttpResponseRedirect('/detail/Section/3/'+str(len(ques)+1)+"/1")
			# 	elif(len(ques2)>=len(Section3.objects.all())):
			# 		print("ending 1")
			# 		return HttpResponseRedirect('/ended')

			# elif(section_no=='2'):
			# 	ques=SolvedQ2.objects.filter(Q(id_no=request.user)).order_by('q_id')
			# 	ques2=SolvedQ3.objects.filter(Q(id_no=request.user)).order_by('q_id')
			# 	if(len(ques)<len(Section2.objects.all()) and id1 != (len(ques)+1)):
			# 		return HttpResponseRedirect('/detail/Section/2/'+str(len(ques)+1)+"/1")
			# 	elif(len(ques2)<len(Section3.objects.all()) and id1 != (len(ques)+1)):
			# 		return HttpResponseRedirect('/detail/Section/3/'+str(len(ques)+1)+"/1")
			# 	elif(len(ques2)>=len(Section3.objects.all())):
			...
			# elif(section_no=='3'):
			# 	ques=SolvedQ3.objects.filter(Q(id_no=request.user)).order_by('q_id')
			# 	if(len(ques)<len(Section3.objects.all()) and id1 != (len(ques)+1)):
			# 		return HttpResponseRedirect('/detail/Section/3/'+str(len(ques)+1)+"/1")
			# 	elif(len(ques)>=len(Section3.objects.all())):
			# 		return HttpResponseRedirect('/ended')
